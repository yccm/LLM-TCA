...
def load_and_check_data(path):
    # Load data
    df = pd.read_csv(f"{path}/MIMIC_syn.csv")
...
X_train, Y0_train, Y1_train, A_train, X_test, Y0_test, Y1_test, A_test = load_and_check_data(path="dataset/MIMIC/MIMIC_tabular")
...
pd.DataFrame(pseudo_outcomes).to_csv("dataset/MIMIC/MIMIC_tabular/pseudo_outcomes.csv", index=False)